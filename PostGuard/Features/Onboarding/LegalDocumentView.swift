...
enum LegalText {
    ...
    7. キーボード拡張について
    本アプリのキーボードは「フルアクセス」を利用しますが、入力テキストはユーザーが「分析」を実行したときに限り、上記の分析目的でのみ送信されます。入力内容の常時収集・記録は行いません。
    ...
    第5条（料金・サブスクリプション）
    一部機能は有料プラン（自動更新サブスクリプション）として提供される場合があります。課金・更新・解約はApp Storeの規定に従います。
